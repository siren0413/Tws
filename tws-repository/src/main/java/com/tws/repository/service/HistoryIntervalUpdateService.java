...
public class HistoryIntervalUpdateService {
    ...
    public void update(JobDataMap map, String symbol, int interval, long startTime, int maxDataPoints) {
        ...
        String requestorId = String.join(".", symbol, String.valueOf(interval));
        String result = GlobalQueues.responseCache.asMap().get(requestorId);
        if (result == null) {
            long lastModified = GlobalQueues.lastModifiedCache.asMap().get(requestorId);
            if (System.currentTimeMillis() - lastModified > 10) {
                startZonedDateTime = getStartTime(symbol, interval, startTime);
                endZonedDateTime = getEndTime(startZonedDateTime, interval, maxDataPoints);
            } else {
                return;
            }
        } else {
...
//        }
    ...
    private ZonedDateTime getEndTime(ZonedDateTime startZonedDateTime, int interval, int maxDataPoints) {
        return startZonedDateTime.plusSeconds(interval * maxDataPoints);
    }

    private ZonedDateTime getStartTime(String symbol, int interval, long startTime) {
