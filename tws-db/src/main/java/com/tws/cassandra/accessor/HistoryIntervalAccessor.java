...
public interface HistoryIntervalAccessor {
    ...
    @Query("SELECT * FROM history.time WHERE symbol = :symbol AND interval = :interval AND time <= :end ORDER BY time DESC LIMIT :dataPoints ALLOW FILTERING")
    @QueryParameters(consistency = "ONE")
    ListenableFuture<Result<HistoryIntervalDB>> getIntervalInTimeForPoints(@Param("symbol") String symbol, @Param("interval") int interval, @Param("end") long start, @Param("dataPoints") int dataPoints);
